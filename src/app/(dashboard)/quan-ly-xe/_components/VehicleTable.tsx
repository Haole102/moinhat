// src/app/(dashboard)/quan-ly-xe/_components/VehicleTable.tsx
"use client";

import { useState } from "react";
import { useVehicleQuery } from "@/hooks/useVehicleQuery";
import { useExpenseQuery } from "@/hooks/useVehicleQuery"; // thêm dòng này
...
export default function VehicleTable({
  ...
  const { data: expenses } = useExpenseQuery(); // thêm dòng này
        ...
        <tbody className="divide-y divide-slate-100 font-medium">
          {vehicles && vehicles.length > 0 ? (
            vehicles.map((vehicle) => {
              /*
               * DERIVE ĐỘNG thay vì đọc field tĩnh vehicle.hasPendingRepair
               * Luôn phản ánh đúng trạng thái thực tế của expenses trong bộ nhớ
               */
